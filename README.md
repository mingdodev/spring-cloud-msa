...
```
> `table.whitelist` 설정 시 테이블명만 지정할 경우, 동일한 이름의 테이블이 다른 스키마에 존재하면 JDBC Source Connector의 메타데이터 조회 과정에서 대상 테이블을 식별하지 못해 오류가 발생할 수 있습니다.
> 이를 방지하기 위해 스키마를 포함한 형태("test.users")로 명시하였습니다.
